    ...
    angular.module('iaPermission', [])
        ...
        .controller('EcpPermissionController', function($rootScope, $scope, $controller, $state, $stateParams, permission, PermissionService, Account) {
            ...
            $scope.medicalDataContent = function(){
                ...
                var covidReclength=medicalRecord.covid_reports.length;
                var surgReclength=medicalRecord.surgical_history.length;
                var hospitalRecLength = medicalRecord.hospital_records.length;

                var actualMedicalData = allergReclength+docReclength+famReclength+immReclength+medconReclength+medicReclength+surgReclength+covidReclength+hospitalRecLength+2;
                ...
                _.forEach(medicalRecord.hospital_records, function(hospitalRec){
                ...
                _.forEach(medicalRecord.covid_reports , function(covidRec){
                    if(!covidRec.pcategory || !covidRec.coviddate  || !covidRec.lotnumber  || !covidRec.mfname || !covidRec.pname || !covidRec.result || !covidRec.srnumber){
                        $scope.hasMedicalData = true;
                        count++;
                    }
                })
            ...
            $scope.recordDataContent = function(){
                ...
                var actualRecordLength = emergencyRecLength+1;
            ...
            $scope.handle = function(postPermissions) {
                ...
                $scope.editpermissionLoading = true;
                if(!$scope.personalCheckbox1 && !$scope.insuranceCheckbox && !$scope.medicalCheckbox && !$scope.lifestyleCheckbox){
                    formattedPermissions['not_all_additional_information'] = true;
                }else{
                    formattedPermissions['not_all_additional_information'] = false;
                }
                
                Account.setEmergencyContactPermission($stateParams.member_id, $stateParams.contact_id, formattedPermissions).then(
                    function(res) {
                        $scope.editpermissionLoading = false;
                        ...
                        if(!$rootScope.previewMode){
                            ...
                            var saveSuccessfullynew = angular.element(document).find('div.member-save-info-permission');
                            saveSuccessfullynew.fadeIn(500,0).slideDown(500);
                            setTimeout(function() {
                                saveSuccessfullynew.fadeOut(500,0).slideUp(500);
        ...
        .factory('PermissionService', ['$q', '$stateParams', 'Account', function($q, $stateParams, Account) {
            ...
            function process(permissions, server) {
               ...
               if(permission.medical.blood===undefined){
                ...
                permission.medical.covid_reports = formatCollection(permissions.medical.covid_reports, server);
                ...
                permission.medical.hospital_records = formatCollection(permissions.medical.hospital_records, server);
            ...
            function filterShareData(profiles, permissions) {
                ...
                filteredData.medical.covid_reports = filterCollection(profiles.medical.covid_reports, permissions.medical.covid_reports);
                ...
                filteredData.medical.hospital_records = filterCollection(profiles.medical.hospital_records, permissions.medical.hospital_records);
