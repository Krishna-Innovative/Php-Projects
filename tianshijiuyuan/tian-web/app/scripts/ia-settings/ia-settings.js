(function() {
    ...
    angular.module('iaSettings', [])
        ...
        .factory('iaSettings', ['$rootScope', '$http', 'API_BASE', 'iaCache', 'locale', 'localeConf', 'localeEvents', 'iaCollection', 'InsuranceType', 'Genders', 'LocationTrack', 'EmergencyChannel', 'SupportLanguages', 'Countries', 'SecurityQuestions','Countriescn', 'Covidresult', 'CovidCategories','vaccineDosagess','PTNFilter4MEM',function ($rootScope, $http, API_BASE, iaCache, locale, localeConf, localeEvents, iaCollection, InsuranceType, Genders, LocationTrack, EmergencyChannel, SupportLanguages, Countries, SecurityQuestions, Countriescn, Covidresult, CovidCategories, vaccineDosagess, PTNFilter4MEM) {
            var language;
            ...
            settings.get = function () {
                ...
                localStorage.removeItem(API_BASE + '/contents');
                return $http.get(API_BASE + '/contents');
            }; 
            ...
            settings.init = function () {
                ...
                $rootScope.globals.settings.cncountry || ($rootScope.globals.settings.cncountry = Countriescn);
                $rootScope.globals.settings.genders || ($rootScope.globals.settings.genders = Genders);
                $rootScope.globals.settings.covidCategories || ($rootScope.globals.settings.covidCategories = CovidCategories);
                $rootScope.globals.settings.covidresult || ($rootScope.globals.settings.covidresult = Covidresult);
                ...
                $rootScope.globals.settings.vaccineDosagess || ($rootScope.globals.settings.vaccineDosagess = vaccineDosagess);
                $rootScope.globals.settings.PTNFilter4MEM || ($rootScope.globals.settings.PTNFilter4MEM = PTNFilter4MEM);
