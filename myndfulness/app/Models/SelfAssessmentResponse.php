...
class SelfAssessmentResponse extends Model
{
    ...
    {  
		...
		if($question_set_id==' ' ||  $question_set_id==''){ 
			$default_question_set_id= DB::table('question_sets')->select('id','name')->where('category', 'Self assesment')->limit(1)->get();
			...
			$result['user_id'] = $post['user_id'];//$post[0]['user_id'];
			$result['category'] = 'Self assesment';
		...
		if(empty($results)){
		$results = DB::table('question_sets')->select('id','name','category')->where('category', 'Self assesment')->limit(1)->get();
		}
		...
		return $result;
			...
			$result['user_id'] = $userId;//$post['user_id'];
			$result['category'] = 'Self assesment';
			$result['previous_set_id'] = 0;
			$result['previous_set_name'] = '';
			$result['completed_set_id'] = '';
