...
use App\Models\SetQuestionOption;
...
class SetQuestionController extends Controller
{
    ...
    public function index($group_id){
        ...
        $group = QuestionSet::where('id',$group_id)->with(['set_questions' => function($q){
	...
	//	print_r($group['name']);
        return view('pages.set-questions.add', ['name' => $group_id,'question_name'=>$group['name']], compact('questions','question_categories','group'));
    }
    public function add()
    {
        $questions = Question::select('id','question')->with(['options' => function($q){
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $is_exist = SetQuestion::where('question_group_id',$request->group_id)->where('group_question_option_id',$request->group_question_option_id)->where('prev_group_question_id', $request->prev_group_question_id)->first();
            if($is_exist){
                return response()->json(['msg' => 'exist'],200);
            }
            $group_question =  SetQuestion::create([
                'question_id' => $request->question_id,
                'question_group_id' => $request->group_id
            ]);
            if($request->group_question_option_id){
                $op = SetQuestionOption::where('id',$request->group_question_option_id)->first();
