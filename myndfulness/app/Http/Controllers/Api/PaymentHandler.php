        ...
        try {
            if ($user->isAppleUser()) {
                ...
                try {
				 ...
				 $purchase_token = $requset->input("purchaseToken");
				 $packageName = $requset->input("packageName");
				 $product_id = $requset->input("productId");
					
                    $subscription_plan = SubscriptionPlan::where('product_uid', $product_id)->first();
					...
					//dd($response);
					$se_response=serialize($response);
