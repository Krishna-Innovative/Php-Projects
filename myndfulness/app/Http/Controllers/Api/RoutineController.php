...
class RoutineController extends Controller
{
    ...
    public function store(UserHabitRouteStoreRequest $request)
    {
        ...
        try {
            ...
            // }
			//\Log::info('entered_today_date (pre-registered)', array($entered_today_date));
        } catch (\Throwable $th) {
            DB::rollback();
            report($th);
            return response()
    ...
    public function update(UserHabitRoutine $routine, UserHabitRouteStoreRequest $request)
    {
        ...
        DB::transaction(function() use ($routine, $request){
			...
			DB::table('user_routine_transactions')->where('status','completed')->where('user_habit_routine_id',$routine->id)->where('user_id',$routine->user_id)->delete();
            $routine->update($request->validated()); // time is not converted into gmt 
            $routine->refresh();
			
            $service = new UserHabitTransactionService($routine, $routine->remind_at_date_time);
            $service->createHabitTransactions();
				//\Log::info('ffd (pre-registered)', array($routine->id));
        });
        return response()
            ->json([
                "message" => "Updated successfully.",
