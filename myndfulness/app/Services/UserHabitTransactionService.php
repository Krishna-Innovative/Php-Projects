...
class UserHabitTransactionService
{
    ...
    private function createTransactions($configs)
    {
        ...
        if($date_time < now()){
			...
			//\Log::info('If time passed (pre-registered)', array($date_time));
        }else{
			//\Log::info('else time passed (pre-registered)', array($date_time));
		}
        // subsctracting and adding same hour is no issue.
        // for
        if($this->newly_created_routine){
            $date_time = $date_time->subHour($configs[1]);
			//\Log::info('If newly_created_routine passed (pre-registered)', array($date_time));
        }else{
			$date_time = $date_time->subHour($configs[1]);
			//\Log::info('else newly_created_routine passed (pre-registered)', array($date_time));
		}
        $data = [];
