...
<div id="feed-page" class="feed-page">
  <div class="container">
	    ...
	    <?php //echo 'vdcjdvcd';
	  echo do_shortcode( '[gmw search_form="7"]' ); ?>
		
      <?php //echo do_shortcode( '[gmw search_results="7"]' ); ?>
	 
		<div class="gmw-results-wrapper grid-gray gmw-pt-grid-gray-results-wrapper pt" data-id="7" data-prefix="pt" id="postdynamic">
			<div class="gmw-results">
	  <?php echo do_shortcode('[ajax_load_more container_type="ul" css_classes="posts-list-wrapper" post_type="beeart" posts_per_page="12"  scroll="false" button_label="Find More"]');?>
	  </div>
		</div>
		<!--<a href="#" class="cp_popup"  data-target="#view_post" data-toggle="modal">View Post</a>-->
	</div>
...
/*.time-block::Before {
	content: '';
	background-image: url('https://beesm.art/wp-content/uploads/2021/12/history.svg');
	height: 22px;
	width: 22px;
...
<script>
$('body').on('change','.LoveCheck',function () {
		    $this = $(this);
            $this.attr("disabled", "disabled");
			var post_id= $this.attr('id');
			var new_value=post_id.split("post_");
			var previous_value=$('#single-post-'+new_value[1]+' .LoveCount').text();
		setTimeout(function() {
		$.ajax({
             type : "POST",
             url : "/wp-admin/admin-ajax.php",
			 data : {action: "reducted_points_after_use_like",'post_id':post_id,'previous_value':previous_value},
			 beforeSend: function(){
				$(".loader").show();
			   },
